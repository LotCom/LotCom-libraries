...
public static class PrintService
{
    ...
    public static async Task<List<Print>> GetAllPrints()
    {
        ...
        else
        {
            ...
            List<PrintDto>? Prints = JsonConvert.DeserializeObject<List<PrintDto>>(Response);
            if (Prints is null)
            {
                throw new JsonException("Could not deserialize the Prints from the response");
            }
            List<Print> Models = Prints.Select(x => PrintMapper.DtoToModel(x));
            Console.WriteLine(Prints.Count);
        }
    }
}